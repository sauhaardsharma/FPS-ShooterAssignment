...
void AShooterProjectile::ProcessHit(AActor* HitActor, UPrimitiveComponent* HitComp, const FVector& HitLocation, const FVector& HitDirection)
{
	...
	// --- 1) Damage Characters (already present)
	...
	// --- 2) Damage Boxes or other destructible actors
	else if (HitActor->IsA(ABoxActor::StaticClass()))
	{
		UGameplayStatics::ApplyDamage(
			HitActor,
			1.0f,   // Or use HitDamage if you want projectile’s damage
			GetInstigator() ? GetInstigator()->GetController() : nullptr,
			this,
			HitDamageType
		);
	...
	// --- 3) Apply physics impulse if it simulates physics
